...
export default function PlaybackDummy() {
  ...
  const pctToDate = (pct: number) => new Date(DAY_START.getTime() + (pct / 100) * (DAY_END.getTime() - DAY_START.getTime()));
 ...
 const handleCameraDrop = async (
  ...
  console.group("🎥 handleCameraDrop");
  console.log("📥 Dropped cameraId:", cameraId);
  ...
  console.log("✅ Camera assigned to slot");

  // 2️⃣ Existing players snapshot (IMPORTANT)
  console.log(
    "🎞 Current players:",
    players.map((p) => ({
      cameraId: p.cameraId,
  ...
  // 3️⃣ Existing player check (camera + date)
  ...
  let blobUrl: string | null | undefined = existingPlayer?.blobUrl;

  if (!blobUrl) {
    console.log("📡 No player found → calling startCamera()");
    blobUrl = await startCamera(cameraId, selectedDate, selectedTimeRange);
    console.log("🆕 blobUrl returned from startCamera:", blobUrl);
  } else {
    console.log("♻️ Reusing existing blobUrl");
  }

    if (blobUrl) {
      console.log("▶️ Stream ready, letting HLS autoplay");
      usePlaybackStore.getState().setIsPlaying(true);
    } else {
      console.warn("⛔ No blobUrl → playback NOT started");
    }
...
const handleSeekToDate = async (date: Date) => {
  ...
  for (const cameraId of slotCameraIds) {
    ...
    if (existingPlayer) {
      if (slotCameraIds[0] === cameraId) {
        console.log("▶️ Load & play existing player:", existingPlayer.blobUrl);
      }
    } else {
      console.log("📡 Fetching new player for camera:", cameraId);
      await startCamera(cameraId, date, { start: nextSegment.startTime, end: nextSegment.endTime });

      const newPlayer = players.find(
        (p) => p.cameraId === cameraId && p.date.toDateString() === date.toDateString()
      );

      if (newPlayer && slotCameraIds[0] === cameraId) {
        console.log("▶️ Load & play new player:", newPlayer.blobUrl);
      }
    }
  }
};
