...
const ConfigureDevicesPage = () => {
  ...
  const handleReboot = async () => {
    ...
    console.log(`DEBUG: Rebooting device ID: ${idToReboot}`);
    try {
      const response = await axios.post(`${API_VAISHALI_URL}/v1/devices/reboot/${idToReboot}`, {}, {
        headers: getAuthHeaders()
      });

      console.log("DEBUG: Reboot API Response:", response.data);

      if (response.data?.success) {
        toast.success(response.data.message || "Device is rebooting...");
      }
    } catch (error) {
            ...
            <>
              <div className="flex items-center justify-between p-4 border-b mb-2">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Button
                    variant="outline"
                    size="sm"
                    className="gap-2 h-9 bg-white font-roboto font-medium"
              ...
              <SettingsTabs
                ...
                <TabsContent value="network" >
                  <Network deviceId={selectedCamera?.id} />
                </TabsContent>
                <TabsContent value="recording" >
                  <Recording deviceId={selectedCamera?.id} />
                </TabsContent>
                <TabsContent value="liveView" >
                  <LiveViewTabs deviceId={selectedCamera?.id} />
                </TabsContent>
